<template>
    <v-toolbar color="teal accent-4" dark fixed app>
      ...
      <v-toolbar-title class="headline font-weight-bold no-selected">{{$store.state.creator.operation === 'local'? $store.state.creator.groupName : $store.state.creator.netBuffer.select}}
      </v-toolbar-title>
      <v-spacer ></v-spacer>
